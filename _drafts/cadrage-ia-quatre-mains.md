...
- **A5 `quartz/plugins/emitters/contentIndex.tsx`.** Préfixer `generateSiteMap` de `<?xml version="1.0" encoding="UTF-8"?>` : mesuré le 31/08 au soir, le sitemap publié ouvre directement sur `<urlset` (déclaration optionnelle en XML 1.0, Google lit les sitemaps Quartz tels quels ; la ligne lève un doute, elle ne corrige pas un défaut). *Prédiction* : `(Invoke-WebRequest …/sitemap.xml).Content.Substring(0, 5)` → `<?xml` après déploiement. Si Search Console affiche encore « Impossible de lire » 48 h après le dépôt du 31/08, re-soumettre après A5.
- **A3(iii) fait par anticipation le 31/08 au soir** : jeton `fXygq_…` posé dans `Head.tsx`, propriété vérifiée, sitemap déposé.

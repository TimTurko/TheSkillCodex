...
**Promotion proposée à l'arbitrage, non écrite seul (§8, topic « nouvelle convention »).** L'item BACKLOG « **tester tout motif générique sur un échantillon avant comptage** » attend sa promotion depuis le 26/08 (suite 4), après deux séances consécutives de fusion de populations (`régul-` / `régulier`, puis `micropython-en` / `micropython-entree-tor`). Aucune occurrence neuve ce jour, **ce qui est en soi un argument à verser** : la règle d'usage tient sans être écrite depuis deux séances.

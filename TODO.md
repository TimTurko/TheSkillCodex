# TODO — TheSkillCodex

> Fichier privé (non publié). Listes courtes et actionnables.

## Prochaines sessions (ordre logique)

### 1. Installation à la maison (PC perso uniquement)
- [ ] Installer Git for Windows sur PC perso
- [ ] Installer Node.js v22 LTS sur PC perso
- [ ] Installer Obsidian sur PC perso
- [ ] Configurer Git (user.name, user.email, init.defaultBranch main)
- [ ] Cloner le dépôt TheSkillCodex sur PC perso
- [ ] Configurer Obsidian Git (mêmes réglages que sur PC pro)
- [ ] Tester un cycle complet : pull → édit test → commit-and-sync → vérifier sur GitHub

> Smartphone : pas d'installation, usage en consultation seule via le site déployé.

### 2. Cadrage pédagogique (à faire AVANT la rédaction massive)
- [ ] **Session "chronopédagogie"** : définir le parcours d'apprentissage type sur un semestre
- [ ] **Trame projet "du début à la fin"** : phases canoniques d'un projet mécatronique (CdCF → analyse → conception → réa → test → revue)
  - But : éviter de sortir du scope, garantir que toutes les fiches s'inscrivent dans cette trame
  - Livrable : un schéma + une fiche `hub/parcours-projet.md` qui devient l'épine dorsale du site
- [ ] **Format des compétences (AA)** : discussion sur la granularité et la convention de tag — en attente validation hiérarchie

### 3. Rédaction de fiches (après cadrage)
- [ ] **Fiche 2 — Cahier des charges fonctionnel** (`proj/cahier-des-charges-fonctionnel.md`)
  - Notion fondatrice, mentionnée 2 fois dans la fiche pilote
  - Couvre `AA-PROJ-C04-CFE3-02` (rédiger un CdCF) — sous réserve convention tags
- [ ] (Puis fiches selon priorité — voir BACKLOG.md)

## Décisions à valider avec la hiérarchie
- [ ] Convention de tags AA (granularité, syntaxe)
- [ ] Niveau d'écart accepté entre référentiel et contenu produit (peut-on aller au-delà ?)
- [ ] Couleurs/charte graphique : faut-il aligner sur la charte école ?

## Tâches techniques en suspens
- [ ] Commit + push de la session du 19/05 pour voir la fiche pilote en ligne
- [ ] Configurer le plugin Templates d'Obsidian (`content/_templates`)
- [ ] Installer **Pandoc + MiKTeX** pour export PDF académique (pas urgent)
- [ ] Vérifier le rendu de la fiche pilote sur smartphone une fois en ligne (vrai test responsive)
- [ ] Produire le schéma d'illustration des conventions de flèches (mentionné dans `schema-bloc-fonctionnel`)
- [ ] Produire/trouver une photo de couveuse annotée (mentionné dans `schema-bloc-fonctionnel`)

## Décisions éditoriales en attente
- [ ] Politique de stockage des images : `content/ressources/img/` ou autre ?
- [ ] Stratégie pour fiches MME/ESE : pages courtes pointant vers collègues, ou ne rien créer ?
- [ ] Glossaire séparé pour acronymes ou fiches courtes individuelles ?

## Fait

*Voir `JOURNAL.md` pour l'historique détaillé.*

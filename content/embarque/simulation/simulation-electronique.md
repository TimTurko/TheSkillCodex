...
**Simuler un système électronique**, c'est en calculer le comportement sur ordinateur — tensions, courants, formes d'onde — **avant de câbler quoi que ce soit**. On vérifie qu'une idée tient, on explore des variantes, on évite de griller un composant sur une fausse manœuvre, et tout cela sans fer à souder. Cette fiche est le **hub d'entrée** vers la simulation : elle pose la **méthode** commune à tous les outils et la façon d'en **lire les résultats**, puis oriente vers le simulateur adapté, des outils analogiques ([[falstad|Falstad]], [[ltspice|LTspice]]) aux simulateurs de cartes à microcontrôleur ([[wokwi|Wokwi]], [[tinkercad|Tinkercad]]).

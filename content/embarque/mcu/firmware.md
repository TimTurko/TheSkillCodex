...
Le firmware n'est pas un logiciel comme celui d'un PC. Il tourne **sans système d'exploitation** (le plus souvent), sur des **ressources comptées** ([[memoire|mémoire]], puissance), en **boucle sans fin**, et il doit être **robuste** : un blocage sur le terrain, sans clavier ni écran pour intervenir, c'est un système mort.
...
5. **Le système temps réel (RTOS).** Quand certaines tâches ont des **échéances strictes** à tenir quoi qu'il arrive, un RTOS (comme FreeRTOS) **préempte** pour garantir les priorités. Puissant mais lourd : à réserver aux cas qui l'exigent, surtout côté [[esp32|ESP32]].

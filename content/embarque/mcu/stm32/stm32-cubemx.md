...
Prendre capture d'écran de *la vue Pinout & Configuration de CubeMX, avec quelques broches affectées en surbrillance (LD2 en GPIO_Output, PA2/PA3 en USART2)*.
...
Prendre capture d'écran de *l'onglet Clock Configuration de CubeMX, montrant l'arbre HSE → PLL → SYSCLK → prédiviseurs AHB/APB avec la fréquence cœur affichée*.
...
Prendre capture d'écran de *le panneau de configuration d'un USART dans CubeMX (Baud Rate 115200, Word Length 8 bits) avec l'onglet NVIC Settings visible*.
...
Prendre capture d'écran de *le moniteur série affichant « tic » répété chaque seconde, à côté de la carte avec LD2 allumée*.

...
> La XIAO est livrée **sans barrettes** : soude les broches dont tu as besoin (attention aux pâtés de soudure, la carte est minuscule). Pour le Wi-Fi/BLE, clipse l'antenne fournie sur le connecteur U.FL — un côté dans le bloc d'abord, puis presse l'autre ; ne tire jamais sur le câble pour la retirer.

## Installer le support et téléverser (Arduino)

1. Installe l'**IDE Arduino** (version stable).
2. **Fichier → Préférences → URL de gestionnaire de cartes supplémentaires**, ajoute :
   `https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json`
3. **Outils → Type de carte → Gestionnaire de cartes**, cherche `esp32` (Espressif) et installe la dernière version (il faut **≥ 2.0.8**).
4. Sélectionne la carte **`XIAO_ESP32S3`** et le port (un `COM3` ou plus sous Windows ; un `usbmodem`/`usbserial` sous macOS — au doute, débranche/rebranche pour voir lequel disparaît).
5. Ouvre **Fichier → Exemples → 01.Basics → Blink** et téléverse : la LED orange se met à clignoter.
...
## Ton premier programme : la LED qui clignote
...
Pour vérifier que les **radios** répondent, lance ensuite un scan Wi-Fi ou BLE — voir [[esp32-wifi|Wi-Fi]] et [[esp32-ble|BLE]].
...
> L'ESP32-S3 gère l'USB **nativement** (pas de puce série dédiée). Si le moniteur reste muet ou renvoie une erreur à l'ouverture, active l'option **« USB CDC On Boot »** dans le menu Outils, puis re-téléverse. Pour l'usage de la liaison série, voir [[esp32-serie|série / moniteur]].
...
- [[xiao-alimentation|XIAO — alimentation]] — alimenter et recharger la carte *(à venir)*

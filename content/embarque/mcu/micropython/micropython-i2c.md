...
Trois rôles : **multiplexer plusieurs capteurs** sur 2 broches (précieux quand les GPIO sont rares) ; **connecter un afficheur** sans monopoliser de broches (un OLED I2C n'en demande que 2) ; **lire/écrire des composants utilitaires** (RTC, EEPROM externe, expanseur de GPIO).
...
I2C exige des **pull-up vers VCC** sur `SDA` et `SCL` (≈ 4,7 kΩ ; 2,2 kΩ en 3,3 V). **La plupart des modules les intègrent** — rien à ajouter pour un premier essai avec un seul module.

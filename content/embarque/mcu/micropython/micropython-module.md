...
Un projet école embarque 5 à 15 modules : capteur (DHT11, HC-SR04, BMP280, MPU6050), driver (L298N, DRV8833), afficheur (OLED I2C), communication, alimentation, utilitaire (RTC DS3231, MicroSD). Tous suivent la même logique de câblage — la maîtriser une fois épargne des heures de tâtonnement.
...
Règle inviolable : **la masse (GND) du module doit être reliée à la masse du Pico**, même quand l'alimentation est séparée. Sans GND commun, les signaux n'ont pas de référence — le module ne répond pas. Discipline utile : rouge pour `VCC`, noir pour `GND`, câbles Dupont courts (< 20 cm) pour I2C/SPI, connecteurs bien enfoncés.
...
**Module contrefait.** Pour les modules courants à bas prix, des contrefaçons passent les premiers tests puis dérivent. Pour un projet sérieux, source connue.
...
Un module bien câblé en début de PoC est un sous-système qu'on ne revisite plus — la validation isolée évite que les problèmes matériels parasitent la mise au point logicielle.

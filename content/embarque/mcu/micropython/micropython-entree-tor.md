...
À l'appui, on voit `1, 1, 0, 0, 1...`. En regardant de près, quelques transitions parasites apparaissent au moment de l'appui (`0, 1, 0, 1`) — c'est le **rebond mécanique**, qui dure quelques millisecondes.
...
while True:
    ...
    if lecture != dernier_etat:
        dernier_changement = ticks_ms()   # instant du changement
        dernier_etat = lecture

    if ticks_diff(ticks_ms(), dernier_changement) > DELAI_REBOND:
        if lecture != etat_stable:         # lecture stable depuis assez longtemps
            etat_stable = lecture
...
```python
        if etat_stable == 0:
            # FRONT DESCENDANT — bouton vient d'etre appuye
            print("Appui detecte !")
```
...
Chaque appui incrémente un compteur et inverse la LED — comportement net, immune au rebond.

...
| Famille | Architecture | Langage / IDE | Conso | E/S & connectivité | Cas d'usage typique |
| --- | --- | --- | --- | --- | --- |
...
| [[stm32\|STM32]] | ARM Cortex-M0 à M7 | STM32CubeIDE, STM32duino, PlatformIO | modes basse conso soignés | périphériques très riches | projets pro/industriels exigeants |
| [[teensy\|Teensy]] | ARM Cortex-M4/M7 (NXP, jusqu'à 600 MHz) | Arduino + Teensyduino | haute performance | E/S rapides, audio | audio, DSP, performance en cadre Arduino |
| [[pic\|PIC]] | 8 / 16 / 32 bits (Microchip) | MPLAB X, compilateurs XC | faible consommation | périphériques variés | industriel, *legacy*, production en très grande série, où la stabilité de la référence et la disponibilité à long terme priment |

Trois repères pour lire ce tableau. La XIAO de Seeed Studio n'est pas une famille de puces mais un **format** : une carte timbre-poste construite (le plus souvent) autour d'un ESP32 — tout ce qui vaut pour l'[[esp32|ESP32]] s'y applique, dans un encombrement minimal. Le Raspberry Pi n'est pas un microcontrôleur mais un ordinateur miniature : il s'impose quand le projet demande un système d'exploitation, du réseau ou du calcul lourd (vision), au prix d'une consommation et d'une complexité supérieures — à ne pas confondre avec le **Raspberry Pi Pico**, qui est, lui, un vrai microcontrôleur (ligne dédiée du tableau) et notamment la carte canonique du parcours [[micropython|MicroPython]]. L'ESP8266 a ouvert la voie du Wi-Fi sur microcontrôleur ; l'ESP32 le supplante aujourd'hui sur presque tous les plans. Il garde son utilité dans deux cas : quand des cartes ESP8266 sont déjà en parc, et pour reprogrammer des objets du commerce qui en sont équipés.
...
5. **Écosystème et disponibilité** — documentation, communauté, bibliothèques et surtout disponibilité du composant dans la durée comptent souvent plus que quelques mégahertz : un MCU bien documenté et qu'on retrouve chez deux distributeurs fait gagner un temps considérable. La maîtrise de l'équipe compte au même titre — une famille que personne n'a jamais programmée se paie en temps d'apprentissage pris sur le projet.
6. **Encombrement et intégration** — quelle place la carte occupe-t-elle dans le produit, et comment s'y fixe-t-elle ? Une carte au format timbre-poste (XIAO) ouvre des intégrations qu'une Uno interdit ; à l'inverse, une carte large aux broches espacées se câble et se dépanne bien plus facilement en phase de prototypage.
